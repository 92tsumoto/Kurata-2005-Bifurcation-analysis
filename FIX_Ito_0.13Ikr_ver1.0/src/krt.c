/* produced by Tsumoto. K 2024.4.27 */
...
int main(int argc, char **argv)
{
	...
	double dist;
	double para;
	double apd[8],rmbp[8],vmax[8],toneapd[8],ttwoapd[8],dvdtmax[8];
		...
		while(ito.rate!=var.endpara){
			...
			while (1){
				...
				for(pe=0;pe<var.l;pe++){
					...
					apd[pe] = 0.0; toneapd[pe] = 0.0; ttwoapd[pe] = 0.0; rmbp[pe] = x[0]; vmax[pe] = -90.0; dvdtmax[pe] = 0.0, v_old = x[0];
					for (j = 0; j< (int)tend; j++){
						t = h*(double)j;
						runge(NN,h,x,t);

						//dvdt_new = (x[0]-v_old)/h;
						dvdt_new = (x[0]-v_old)/(h/2.0/M_PI);
						if(x[0] > vmax[pe]){
							vmax[pe] = x[0];
						}
						//if(dvdt_new < dvdtmax){
						if(dvdt_new > dvdtmax[pe]){
							dvdtmax[pe] = dvdt_new;
							toneapd[pe] = time*var.BCL/2.0/M_PI;
						}
						if(dvdt_new < 0 && x[0] >= (vmax[pe] - 0.9*(vmax[pe] - rmbp[pe]))) ttwoapd[pe] = time*var.BCL/2.0/M_PI;
			...
			if( count < new.iterm && var.istop==0 ){
				printf("iter=%d rate=%10.9lf x: ",count,ito.rate);
				fprintf(fp2,"iter=%d rate=%10.9lf x: ",count,ito.rate);
				for(k=0;k<N;k++){
					x[k]=new.xn0[k];
					printf("%10.9e / ",x[k]);
				...
				//apd[] = ttwoapd - toneapd;
				printf("[ %lf ] / ",ttwoapd[0]-toneapd[0]);
				fprintf(fp2,"[ %lf ] / ",ttwoapd[0]-toneapd[0]);
				fflush(fp2); 

				if(var.l>1){
					fprintf(fp4,"iter=%d rate=%10.9lf x: ",count,ito.rate);
					for(k=0;k<N;k++){
						fprintf(fp4,"%10.9e / ",x2[k]);
					}
					//apd = ttwoapd - toneapd;
					for(k=0;k<var.l-1;k++){
					printf("[ %lf ] / ",ttwoapd[k+1]-toneapd[k+1]);
					fprintf(fp4,"[ %lf ] \n",ttwoapd[k+1]-toneapd[k+1]);
					fflush(fp4); 
					}
				}
				/* Solve eigenproblem */
