...
void val_consts(FILE *fp1)
{
	...
	// Rapid delayed rectifier potassium current (Ikr)
		...
		if(var.celltype == 0){
			ikr.Gkr = 0.012*(1.0/1.3)*ikr.rate;  //Endo (based on O'Hara-Rudy 2011 model)
		} else if(var.celltype == 1){
			ikr.Gkr = 0.012*ikr.rate;  		// Epi (default for Kurata 2005 model)
		} else if(var.celltype == 2){
			ikr.Gkr = 0.012*(0.8/1.3)*ikr.rate;  // Mid (based on O'Hara-Rudy 2011 model)
		}
