...
// CDI 
	...
	ical.gfCa_inf = x[12]/(ical.kmmca+x[12]);
...
void comp_ikr (double x[])
{
	...
	ikr.ik = ikr.Gkr*x[7]*ikr.piinf*(x[0]-var.Ek);
...
void comp_iks (double x[])
{
	...
	iks.ik = iks.Gks*(x[0]-iks.Ek)*x[8]*x[8];
...
void comp_inak (double x[])
{
	...
	inak.rhonak = inak.Trhonak[iV]*d2 + inak.Trhonak[iV+1]*d1;
