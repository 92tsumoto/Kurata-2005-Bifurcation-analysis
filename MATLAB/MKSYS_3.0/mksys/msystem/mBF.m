...
%
	...
	if fin == -1
		...
		[ipx,ipy,dipx,dipy,endipx,endipy,tmp_mu,l,nnn] = init_binput(tmp_mu);
...
fprintf('period kmax au nnn reverse: %d %d %f %d %d\n',l,kmax,au,nnn,reverse_sw);
fprintf('x : %e, %e\n',x(1),x(2));
fprintf('i) para(1) para(2) / x | (c.m.)re im abs | (Jacobian)\n');
fprintf('-------------------\n');
if(iosave == 1)
	fprintf(bf_out1,'# %d %d %d %f %f %d %d\n',ip(1),ip(2),bf_type,mu,epsx,l,kmax);
	fprintf(bf_out1,'# %f %f\n',para(ip(1)),para(ip(2)));
end
...
while forcing_stop
	...
	if bf_type == 3
		[s_ind,iterN,x,para,chara,det_value,theta]=Newton_bf_NS(l,x,para,ip,theta);
	else	
		[s_ind,iterN,x,para,chara,det_value]=Newton_bf(l,x,para,ip);
	end
