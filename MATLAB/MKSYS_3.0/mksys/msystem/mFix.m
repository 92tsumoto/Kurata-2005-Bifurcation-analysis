...
%
	...
	if fin == -1
		...
		[sname,ipx,ipy,dipx,dipy,endipx,endipy,l,nnn,istopbf]=init_finput();
...
while forcing_stop
	...
	%
		[s_ind,iterN,x,para,chara,det_value]=Newton_fix(l,x,para);
	...
	switch s_ind
			...
			if istopbf == 2
				...
				if abs(eig_dist) < seigtol
					...
					if isequal(para(ip),ini_para)
						fprintf('Please check the initial settings, e.g., Period, initail states, parameter, etc.\n');
					end
		...
		case 3 % The case that the function msysvar returned a error value
			fprintf('\n');
			fprintf('Now, the calculation is terminated.\n');
			fprintf('This is because an error value from the function msysvar is returned.\n');
			backup_out(stok);
			return;
