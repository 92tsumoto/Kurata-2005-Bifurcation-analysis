...
% debuging switch
% If you make debug, should set the following swith. 
% This switch is generally defaulted "off".
debug_sw = 0;
...
%
	...
	if fin == -1
		...
		[sname,ipx,ipy,dipx,dipy,endipx,endipy,l,m,nnn,tau1,istopbf]=init_finput();
...
while forcing_stop
	...
	%
		[s_ind,iterN,x,para,chara,det_value]=Newton_fix(l,m,x,para,tau1);
	...
	switch s_ind
		case 0 % convergence case
			...
			if sn_flag == 1	
				...
				if abs(eig_dist) < seigtol && real(stok_chara(idx)) > 0
