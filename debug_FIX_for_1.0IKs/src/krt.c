	...
	for (ii = 0; ii < var.datas; ii++){
		...
		while (1){
			...
			for(pe=0;pe<var.l;pe++){
				...
				//time=0.0; 
				tend=(double)var.m*var.ndis;
				for (j = 0; j< (int)tend; j++){
					t = h*(double)j;
					runge(NN,h,x,t);
					fprintf(fp4,"%10.9lf ",time/var.omega);
					for(w=0;w<N;w++){
						if(w!=N-1){
							fprintf(fp4,"%10.9lf ",x[w]);
						} else {
							fprintf(fp4,"%10.9lf\n",x[w]);
						}
					}
				...
				// added for debug
				if(var.fdebug==1){
					printf("t=%10.9lf ",time/var.omega);
					for(w=0;w<N;w++){
						if(w!=N-1){
							printf("%10.9lf ",x[w]);
						} else {
		...
		if( count < new.iterm && var.istop==0 ){
			printf("iter=%d rate=%lf x: ",count,ikr.rate);
			fprintf(fp2,"iter=%d rate=%lf x: ",count,ikr.rate);
			for(k=0;k<N;k++){
				x[k]=new.xn0[k];
				printf("%10.9e / ",x[k]);
